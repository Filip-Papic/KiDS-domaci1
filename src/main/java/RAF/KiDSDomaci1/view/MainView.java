...
public class MainView {
	...
	private Pane center;
	private static ListView<String> results;
	...
	private static XYChart.Series<Number, Number> chart;

	public void initMainView(BorderPane borderPane, Stage stage) {

		this.stage = stage;

		stage.setOnCloseRequest(e -> App.closeWindow());
	...
	private void initRight(BorderPane borderPane) {
		...
		center.getChildren().add(singleResult);
	...
	private void getSingleResult() {
		String selected = String.valueOf(results.getSelectionModel().getSelectedItems());
		chart = CacheOutput.getCharts(selected);
	...
	private void sumResults() {
		...
		result.ifPresent(res -> {
			...
			String selected = String.valueOf(results.getSelectionModel().getSelectedItems());
			chart = CacheOutput.sumCharts(selected, res);
			System.out.println("Selected: " + selected);
			lineChart.getData().clear();
			lineChart.getData().addAll(chart);

			results.getItems().add(res);
		});
	}
	...
	private void addCruncher() {
		...
		result.ifPresent(res -> {
			try {
				...
				CounterCruncher cruncher = new CounterCruncher(arity);
	...
	public static ListView<String> getResults() {
		return results;
	}
